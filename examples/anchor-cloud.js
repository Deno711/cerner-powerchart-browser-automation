/**
 * anchor-cloud.js — AnchorBrowser cloud example for Cerner PowerChart
 *
...
const { login_powerchart, search_patient } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Cerner PowerChart automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_powerchart(page, {});
